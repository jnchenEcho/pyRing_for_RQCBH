...
# 20250908, chen, reweighting Mf prior for RQC
try:
    import configparser
except ImportError:
    ...
    def extract_gwtc_data(self, params):
        ...
        with h5py.File(self.file_path, 'r') as f:
            ...
            for param in params:
                ...
                # Handle final_* parameters 
                if param.startswith('final_'):
                    ...
                    # Non-final_* parameter (strict check)
                    hdf5_key = param_mapping[param]
                    try:
                        data = BBH[hdf5_key][()]
                        # Apply transformations if needed (e.g., tilt angles)
                        if param in ['tilt1', 'tilt2', 'inclination'] and 'GWTC' in self.file_path and 'GWTC4' not in self.file_path:
                            data = np.arccos(data)
                        result[param] = data
                    except (KeyError, ValueError):
                        raise KeyError(f"Key '{hdf5_key}' not found in HDF5 file.")
...
def set_prefix(warning_message=True):
    ...
    # Check environment
    try:
        # Chen, 20250706, since the path exits an additional filename "pyRing"
        prefix = os.path.join(os.environ['PYRING_PREFIX'])
...
def qnm_interpolate_RQC(s,l,m,n):

    """

    This function interpolates the complex frequencies of the QNM modes for the RQCBH.
    ...
    interp_r : interp1d object
        Interpolant function of the real part of the complex frequency.
    interp_i : interp1d object
        Interpolant function of the imaginary part of the complex frequency.
    ...
    try:
        PYRING_PREFIX = set_prefix()
        if (m<0): af, RQC_alpha, w_r, w_i = np.loadtxt(os.path.join(PYRING_PREFIX,'data/NR_data/RQCBH_data/s{}l{}'.format(s, l),'n{}l{}mm{}.dat'.format(n, l, np.abs(m))), unpack=True)
        else    : af, RQC_alpha, w_r, w_i = np.loadtxt(os.path.join(PYRING_PREFIX,'data/NR_data/RQCBH_data/s{}l{}'.format(s, l),'n{}l{}m{}.dat'.format(n, l, m)), unpack=True)
    except:
        raise Exception("Loading RQCBH data failed. Exiting.".format(traceback.print_exc()))

    coords   = np.column_stack((af,RQC_alpha))
    interp_r = LinearNDInterpolator(coords, w_r)
    interp_i = LinearNDInterpolator(coords, w_i)
...
def construct_interpolant(param_name, N_bins=32, par_file = 'ringdown/random_parameters_interpolation.txt'):
    ...
    try:
        ...
        PYRING_PREFIX      = set_prefix()
        #20250908, chen,  reweighting Mf prior for RQC
        par_file = str(Config.get("Priors", 'prior-reweight-samples')) 
...
class RemnantModel:
    ...
    def UIB_final_state_fits_aligned_spins(self, m1, m2, chi1, chi2):

        """

        Compute remnant parameters in the spin-aligned case using UIB fits (https://arxiv.org/abs/1611.00332).
        ...
        chi1 : float or array-like
            Signed aligned spin component (z-component, aligned to the orbital angular momentum) of the first BH (can be positive or negative)
        chi2 : float or array-like
            Signed aligned spin component (z-component, aligned to the orbital angular momentum) of the second BH (can be positive or negative)
    ...
    def UIB_final_state_fits_precessing_spins(self, m1, m2, chi1, chi2, tilt1, tilt2):

        """
        Compute remnant parameters in the spin-precessing case using UIB fits (https://arxiv.org/abs/1611.00332).
        ...
        m2 : float or array-like  
            Mass of the second BH
        chi1 : float or array-like
            Magnitude (absolute value) of spin of the first BH (only positive)
        chi2 : float or array-like
            Magnitude (absolute value) of spin of the second BH (only positive)
    ...
    def NRsur_final_state_fits(self, m1, m2, chi1, chi2, tilt1, tilt2, phi_12, phi_jl, theta_jn, phase):
        ...
        # FIXME: Pass f_ref and approximant as to be coherent with IMR PE.
        fits = NRSur_fit(
