...
from pyRing.utils      import RemnantModel
...
class KerrModel(RingDownModel):
    ...
    def __init__(self, **kwargs):
        ...
        # RQCBH, chen, 202505
        self.RQC                    = kwargs['RQC']
        self.RQC_Mf_gaussian_prior  = kwargs['RQC-Mf-gaussian-prior']
        ...
        #  RQCBH, chen, 202505
            ...
            utils.review_warning()
            RQC_alpha_single_bounds = [0.0, 27/16]
            RQC_alpha_name = 'RQC_alpha'
            try:
                self.fixed_params[RQC_alpha_name] = self.Config.getfloat("Priors",'fix-'+RQC_alpha_name)
            except(configparser.NoOptionError):
                try:
                    single_bounds = [self.Config.getfloat("Priors",RQC_alpha_name+'-min'),self.Config.getfloat("Priors",RQC_alpha_name+'-max')]
                except(KeyError, configparser.NoOptionError, configparser.NoSectionError):
                    single_bounds = RQC_alpha_single_bounds
                print(('{} : [{},{}]'.format(RQC_alpha_name.ljust(len('cos_altitude')), single_bounds[0], single_bounds[1])))
                if (kwargs['inject-RQC_alpha']):
                    if (not(single_bounds[0] <= kwargs['injection-parameters'][RQC_alpha_name] <= single_bounds[1])): utils.print_out_of_bounds_warning(RQC_alpha_name)
                self.names.append(RQC_alpha_name)
                self.bounds.append(single_bounds)
        ...
        if(kwargs['qnm-fit'] == 0):
            ...
            for (s,l,m,n) in full_modes:
                ...
                # RQCBH, chen, 202505
                elif(self.RQC):    interpolate_freq, interpolate_tau = utils.qnm_interpolate_RQC(s,l,m,n)
    ...
    def log_prior(self,x):
        ...
        if(self.RQC):
            ...
            af_tmp = get_param_override(self.fixed_params, x, 'af')
            RQC_alpha_tmp = get_param_override(self.fixed_params, x,'RQC_alpha')

            if (9-8*af_tmp**2 < 0) : 
                return -np.inf
            if not( RQC_alpha_tmp < 1.0/128*(np.sqrt(9 - 8*af_tmp**2) + 3)**2 * (-4*af_tmp**2 + np.sqrt(9-8*af_tmp**2) + 3) ): 
                return -np.inf
            
        if(self.RQC_Mf_gaussian_prior):
            utils.review_warning()
            Mf_tmp = get_param_override(self.fixed_params,x,'Mf')
            Mf_mean = self.Config.getfloat("Priors",'fix-RQC-Mf-gaussian_prior_mean')
            Mf_std  = self.Config.getfloat("Priors",'fix-RQC-Mf-gaussian_prior_std')
            logPrior += -0.5*((Mf_tmp - Mf_mean)/Mf_std)**2 - np.log(Mf_std) - 0.5*np.log(2*np.pi)
    ...
    def get_waveform(self,x):
        ...
        # RQCBH, chen, 202505
        elif(self.RQC):     TGR_parameters['RQC_alpha']    = get_param_override(self.fixed_params,x,'RQC_alpha')
...
class MMRDNPModel(RingDownModel):
        ...
        def get_waveform(self,x):
            ...
            MMRDNP_params['Mf'], MMRDNP_params['af'] = RM.UIB_final_state_fits_aligned_spins(MMRDNP_params['m1'], MMRDNP_params['m2'], MMRDNP_params['chi1'], MMRDNP_params['chi2'])
...
def main():
    ...
    if(input_par['sampler']=='cpnest'):
        sampler_params={'verbose'                     : 2,
                        ...
                        'seed'                        : 1234,
                        'resume'                      : 1,
                        'periodic_checkpoint_interval': 10800,
                        }
    ...
    sampler_outfiles = []
    if input_par['sampler'] == 'cpnest':
        sampler_outfiles = [os.path.join(input_par['output'],'Nested_sampler/posterior.dat'),
                    os.path.join(input_par['output'],'Nested_sampler/chain_{}_{}.txt_evidence.txt'.format(sampler_params['nlive'], sampler_params['seed']))
                    ]
    elif input_par['sampler'] == 'raynest':
        sampler_outfiles = [os.path.join(input_par['output'],'Nested_sampler','cpnest.h5')]
    else:
        raise RuntimeError(f"Unknown sampler {input_par['sampler']}")

    sampler_complete = all(os.path.exists(f) for f in sampler_outfiles)
    if sampler_complete:
        print(f"Sampler output files {sampler_outfiles} already exist, not re-running sampler.")

    if (input_par['run-type']=='noise-estimation-only'):
        print('* Noise estimation done, `noise-estimation-only` option was selected. Exiting.')
        exit()

    if (input_par['run-type']=='full')  and not sampler_complete:
    ...
    elif (input_par['run-type'] == 'post-processing') or sampler_complete:
    ...
    try:
        if((input_par['template']=='Damped-sinusoids') or (input_par['template']=='Morlet-Gabor-wavelets')):
            ...
            if (input_par['template']=='Kerr'):
                ...
                # RQCBH, chen, 202505
                if(input_par['RQC']):
                    try:                                                                                   plots.RQC_intrinsic_corner(x, **input_par)
                    except(KeyError, ValueError, configparser.NoOptionError, configparser.NoSectionError): print("\n* Warning: Plot of intrinsic Kerr-Newman parameters failed with error: {}.".format(traceback.print_exc()))  
