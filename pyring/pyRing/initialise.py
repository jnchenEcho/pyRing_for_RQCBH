...
def read_config(Config, config_file):
    ...
    input_par={
               ...
               'free-time-prior'            : 0,
               'mf-time-prior'              : 0.0,
               'RQC-Mf-gaussian-prior'      : 0,
               'prior-reweight-samples'     : '',

               #===========================#
               # Default plots parameters. #
               #===========================#

               'imr-samples'                : ''   ,
               'imr-remnant-fits'           : 'UIB',
               'downsample-N'               : -1   ,
...
def read_injection_parameters(input_par, Config):
    ...
    if (input_par['injection-approximant']=='MMRDNP'):
        ...
        injection_parameters['Mf'], injection_parameters['af'] = RM.UIB_final_state_fits_aligned_spins(injection_parameters['m1'], injection_parameters['m2'], injection_parameters['chi1'], injection_parameters['chi2'])
...
to be intended as part of the default value.
       ...
       ***************************************************
               ...
               inject-RQC_alpha                 Flag to inject a Kerr-Newman waveform. Default: 0.
               ...
               RQC                        Flag to allow for the inclusion of RQC, named `RQC_alpha`. Default: 0.
       ...
       ****************************************************
               ...
               RQC_Mf_gaussian_prior      Flag to select a Gaussian prior on Mf when sampling RQC_alpha. Default: 0.
               prior-reweight-samples     File containing the reweighting prior samples, produced by personal demand. Default: ''.
       ...
       **************************************************************
               ...
               periodic_checkpoint_interval Option to checkpoint the sampler every `periodic_checkpoint_interval` seconds.  Default: 10800 (3hrs).
    ...
    substituted at each NS step. Requires N_ev << nlive. Also n_cpu = nnest+nensemble. Default: 1.
       ...
       **************************************************
                ...
                imr-samples               File containing the posterior samples from an IMR run to compare against ringdown values in plots. Needs to be either a full posterior in the GWTC-1 format or the marginalised posterior on Mf-af, with two columns and the ['Mf', 'af'] header. Default: ''.
