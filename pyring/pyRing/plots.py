...
# RQCBH, chen 20250707
def RQC_intrinsic_corner(x, **input_par):

    """

    Create the corner plot of the intrinsic parameters of the RQCBH model.

    Parameters  
    ----------

    x : array
        Array containing the samples of the intrinsic parameters of the RQCBH model.
    input_par : dictionary
        Dictionary containing the input parameters of the run.
    ...
    pos             = np.column_stack( (x['Mf'], x['af'], x['RQC_alpha']) )
    injected_values = None

    if (input_par['inject-RQC_alpha']):
        injected_values = [input_par['injection-parameters']['Mf'], input_par['injection-parameters']['af'], input_par['injection-parameters']['RQC_alpha']]
    ...
    mode_corner(pos,
                ...
                use_math_text = True,
                truths        = injected_values,
                filename      = os.path.join(input_par['output'],'Plots/Parameters/RQC_intrinsic_corner.png'))
...
def read_Mf_af_IMR_posterior(Config, **input_par):
    ...
    # Attempt to read final state posterior from IMR PE samples
    try:
        ...
        if not file.endswith('.txt'):
            ...
            final_params = ['final_mass', 'final_spin', 'final_mass_non_evolved', 'final_spin_non_evolved']
            data         = BBH.extract_gwtc_data(final_params)

            # We are assuming that if final mass was internally computed by the IMR PE, so is final spin (otherwise the try-except will throw an error)
            if( (data['final_mass'] is not None) or (data['final_mass_non_evolved'] is not None)):
                sys.stdout.write('\n* Using precomputed final mass and spin from posterior samples.\n')
                
                # Default to evolved ones as they are more accurate, otherwise fallback to non-evolved
                Mf_d = data['final_mass'] if data['final_mass'] is not None else data['final_mass_non_evolved']
                af_d = data['final_spin'] if data['final_spin'] is not None else data['final_spin_non_evolved']

            else:
                sys.stdout.write('\n* Final mass/spin not found in samples, calculating from progenitors parameters.\n')
                # Define parameters to extract
                params = ['m1', 'm2', 'chi1', 'chi2', 'tilt1', 'tilt2', 'phi_12', 'phi_jl', 'theta_jn', 'phase']
                data   = BBH.extract_gwtc_data(params)

                # FIXME: this step should be done in the initialisation and propagated here
                ...
                #FIXME: read IMR approximant from posterior and use that consistently

                RM = RemnantModel()
                if   remnant_fits == 'UIB'  : Mf_d, af_d = RM.UIB_final_state_fits_precessing_spins(data['m1'], data['m2'], data['chi1'], data['chi2'], data['tilt1'], data['tilt2'])
    ...
    if(downsample_N > 0):
        print(f'\n* Downsampling the final mass/spin to {downsample_N} samples.\n')
        idx        = np.random.choice(len(Mf_d), size=downsample_N, replace=False)
        Mf_d, af_d = Mf_d[idx], af_d[idx]
