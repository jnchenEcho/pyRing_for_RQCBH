...
cdef class QNM:
    ...
    # RQCBH, chen 20250710
    f_RQC: float
        Returns the real part of the QNM frequncy for the RQCBH.
    tau_RQC: float
        Returns the imaginary part of the QNM frequency for RQCBH.
    ...
    #RQCBH, chen 20250710
    cpdef double f_RQC(self, double M, double a, double RQC_alpha): return self.omegar_interp(a, RQC_alpha)*self.prefactor_freq*(1.0/M)
    ...
    #RQCBH, chen 20250710
    cpdef double tau_RQC(self, double M, double a, double RQC_alpha): return -1.0/(self.omegai_interp(a, RQC_alpha)*self.prefactor_tau*(1.0/M))
...
cdef class KerrBH:
  ...
  """
    ...
    # RQCBH, chen 20250710
    | RQC                 : Flag to turn on the RQCBH model. Default: 0.
  ...
  def __cinit__(self,
                ...
                # RQCBH, chen 20250710
                unsigned int RQC                 = 0,
                unsigned int RQC_Mf_gaussian_prior = 0,
    ...
    for (s,l,m,n) in modes_full:
        # RQCBH, chen 20250710, the perturbation type for RQCBH actually is s=0 instead of s=2, 
        # which means that we use the Klein-Gordan equation for massless scalar field rather than modified TE for tensorial perturbation.
        # Thus, s should be 0 in strain or GW expression, but we but we still use s=2 for analysis
        assert not(not(s==2) and (self.AreaQuantization or self.qnm_fit or self.charge or self.braneworld or self.RQC)), "Non-tensorial modes (s={} was selected) are incompatible with using either a fit for QNM or the area quantization proposal or a BH charge or braneworld corrections.".format(s)
    ...
    for (s,l,m,n),a in self.amps.items():
        ...
        if(self.qnm_cached is not None):
            ...
            # RQCBh, chen 20250710
                ...
                RQC_alpha  =self.TGR_params['RQC_alpha']
                freq       = self.qnms[(s,l,m,n)].f_RQC(self.Mf, self.af, RQC_alpha) 
                tau        = self.qnms[(s,l,m,n)].tau_RQC(self.Mf, self.af, RQC_alpha)
                corr_dfreq = 1.0
                corr_dtau  = 1.0
